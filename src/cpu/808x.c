 ...
 *		808x CPU emulation.
 *
 * Version:	@(#)808x.c	1.0.10	2019/02/11
 *
...
opcodestart:
	...
	switch (opcode) {
		...
		case 0xa1:
			...
			do_access(1, bits);
			if (opcode & 1)
				AX = readmemw((ovr_seg ? *ovr_seg : ds), do_addr);
			else
				AL = readmemb((ovr_seg ? *ovr_seg : ds) + do_addr);
			cpu_wait(1, 0);
			break;
		...
		case 0xa3:
			...
			do_access(7, bits);
			if (opcode & 1)
				writememw((ovr_seg ? *ovr_seg : ds), do_addr, AX);
			else
				writememb((ovr_seg ? *ovr_seg : ds) + do_addr, AL);
			break;
		...
		case 0xef:
			...
			if ((opcode & 2) == 0) {
				...
				if ((opcode & 1) && is8086 && !(cpu_data & 1)) {
					AX = inw(cpu_data);
					cpu_wait(4, 1);		/* I/O do_access and wait state. */
				} else {
					AL = inb(cpu_data);
					if (opcode & 1)
						AH = inb(temp + 1);
