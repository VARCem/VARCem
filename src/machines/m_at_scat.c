 ...
 *
 * Version:	@(#)m_at_scat.c	1.0.8	2018/05/06
 *
...
static int scatsx_mem_conf_val[33] = {
    0x00, 0x01, 0x03, 0x04, 0x05, 0x08, 0x06, 0x06,
...
get_scat_addr(uint32_t addr, scat_t *p)
{
	...
	if (mem_size < 2048 && ((scat_regs[SCAT_DRAM_CONFIGURATION] & 0x0F) > 7 || (scat_regs[SCAT_EXTENDED_BOUNDARY] & 0x40) != 0))
		...
		addr &= ~0x600000;
		if(mem_size > 2048 || ((uint32_t)mem_size == 2048 && (scat_regs[SCAT_DRAM_CONFIGURATION] & 0x0F) < 6))
	...
	if ((scat_regs[SCAT_EXTENDED_BOUNDARY] & 0x40) == 0 && (scat_regs[SCAT_DRAM_CONFIGURATION] & 0x0F) == 3 && (addr & ~0x600000) >= 0x100000 && (addr & ~0x600000) < 0x160000)
		addr ^= (uint32_t)mem_size < 2048 ? 0x1F0000 : 0x670000;
    } else {
...
scat_memmap_state_update(void)
{
	...
	mem_mapping_set_exec(&scat_4000_9FFF_mapping[i], addr < ((uint32_t)mem_size << 10) ? ram + addr : NULL);
    }

    addr = get_scat_addr(0xA0000, NULL);
    mem_mapping_set_exec(&scat_A000_BFFF_mapping, addr < ((uint32_t)mem_size << 10) ? ram + addr : NULL);
    for (i=0;i<6;i++) {
	addr = get_scat_addr(0x100000 + (i << 16), NULL);
	mem_mapping_set_exec(&scat_shadowram_mapping[i], addr < ((uint32_t)mem_size << 10) ? ram + addr : NULL);
    }
	...
	case 0x23:
		switch (scat_index) {
			...
			case SCAT_EMS_CONTROL:
				if (val & 0x40) {
					if (val & 1) {
						io_sethandler(0x0218, 3, scat_read, NULL, NULL, scat_write, NULL, NULL,  NULL);
						io_removehandler(0x0208, 3, scat_read, NULL, NULL, scat_write, NULL, NULL,  NULL);
					} else {
						io_sethandler(0x0208, 3, scat_read, NULL, NULL, scat_write, NULL, NULL,  NULL);
						io_removehandler(0x0218, 3, scat_read, NULL, NULL, scat_write, NULL, NULL,  NULL);
					}
				} else {
					io_removehandler(0x0208, 3, scat_read, NULL, NULL, scat_write, NULL, NULL,  NULL);
					io_removehandler(0x0218, 3, scat_read, NULL, NULL, scat_write, NULL, NULL,  NULL);
			...
			case SCAT_EXTENDED_BOUNDARY:
				...
				if ((val ^ scat_regs[SCAT_EXTENDED_BOUNDARY]) & 0x40)
					scat_map_update = 1;
				scat_reg_valid = 1;
				break;
		...
		if (scat_reg_valid)
			...
			pclog("SCAT: attempted to write unimplemented SCAT register %02X at %04X:%04X\n", scat_index, val, CS, cpu_state.pc);
		if (scat_shadow_update)
			scat_shadow_state_update();
		if (scat_map_update)
			scat_memmap_state_update();
//		pclog("SCAT: write Register %02X to %02X at %04X:%04X\n", scat_index, val, CS, cpu_state.pc);
		break;
...
//			pclog("SCAT: write EMS Control Port %04X to %02X at %04X:%04X\n", port, val, CS, cpu_state.pc);
			if ((scat_regs[SCAT_VERSION] & 0xF0) == 0)
			...
			if ((scat_regs[SCAT_EMS_CONTROL] & 0x80) && (scat_stat[index].regs_2x9 & 0x80)) {
				...
				if (virt_addr < ((uint32_t)mem_size << 10))
					...
					mem_mapping_set_exec(&scat_mapping[index], NULL);
				flushmmucache();
			}
		}
		break;
		...
		switch (scat_index) {
			...
			case SCAT_DRAM_CONFIGURATION:
				if ((scat_regs[SCAT_VERSION] & 0xF0) == 0)
					...
					if (mem_size != 1024)
						val = (scat_regs[scat_index] & 0xe0) | scatsx_mem_conf_val[mem_size >> 9];
					else
...
//		pclog("SCAT: read Register %02X at %04X:%04X\n", scat_index, CS, cpu_state.pc);
		break;
	...
	case 0x218:
		if ((scat_regs[SCAT_EMS_CONTROL] & 0x41) == (0x40 | ((port & 0x10) >> 4))) {
//			pclog("SCAT: read EMS Control Port %04X at %04X:%04X\n", port, CS, cpu_state.pc);
			if ((scat_regs[SCAT_VERSION] & 0xF0) == 0)
				index = scat_ems_reg_2xA & 0x1F;
			else
				index = ((scat_ems_reg_2xA & 0x40) >> 4) + (scat_ems_reg_2xA & 0x3) + 24;
...
static uint8_t
mem_read_scatems(uint32_t addr, void *priv)
{
...
static void
mem_write_scatems(uint32_t addr, uint8_t val, void *priv)
{
    scat_t *stat = (scat_t *)priv;
