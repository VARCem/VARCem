 ...
 *
 * Version:	@(#)pnp.c	1.0.1	2021/11/11
 *
 *
...
#define CHECK_CURRENT_LD()	if (!dev->current_ld) { \
					DEBUG("ISAPnP: No logical device selected\n"); \
					break; \
				}
...
#define CHECK_CURRENT_CARD()	if (1) { \
					...
					if (!card) { \
						DEBUG("ISAPnP: No card in CONFIG state\n"); \
						break; \
					} \
				}
...
static uint8_t
isapnp_read_rangecheck(uint16_t addr, priv_t priv)
{
...
static uint8_t
isapnp_read_data(uint16_t addr, priv_t priv)
{
...
static void
isapnp_write_addr(uint16_t addr, uint8_t val, priv_t priv)
{
    isapnp_t *dev = (isapnp_t *) priv;
...
static void
isapnp_write_data(uint16_t addr, uint8_t val, priv_t priv)
{
    isapnp_t *dev = (isapnp_t *) priv;
    isapnp_card_t *card;
    isapnp_device_t *ld;
		...
		priv_t priv)
{
    ...
    isapnp_card_t *card;
    card = (isapnp_card_t *) mem_alloc(sizeof(isapnp_card_t));
...
void
isapnp_update_card_rom(priv_t priv, uint8_t *rom, uint16_t rom_size)
{
...
void
isapnp_enable_card(priv_t priv, uint8_t enable)
{
    isapnp_t *dev = (isapnp_t *) device_get_priv(&isapnp_device);
    if (!dev)
	return;
...
void
isapnp_set_csn(priv_t priv, uint8_t csn)
{
    isapnp_card_t *card = (isapnp_card_t *) priv;
...
void
isapnp_set_device_defaults(priv_t priv, uint8_t ldn, const isapnp_device_config_t *config)
{
    isapnp_card_t *card = (isapnp_card_t *) priv;
...
void
isapnp_reset_device(priv_t priv, uint8_t ldn)
{
    isapnp_card_t *card = (isapnp_card_t *) priv;
    isapnp_device_t *ld = card->first_ld;
...
isapnp_close(priv_t priv)
{
    ...
    io_removehandler(0x279, 1, 
		     NULL, NULL, NULL, isapnp_write_addr, NULL, NULL, dev);
    io_removehandler(0xa79, 1, 
		     NULL, NULL, NULL, isapnp_write_data, NULL, NULL, dev);

    free(dev);
}

static priv_t
isapnp_init(const device_t *info, UNUSED(void *parent))
{
    isapnp_t *dev;
    
    dev = (isapnp_t *)mem_alloc(sizeof(isapnp_t));
    memset(dev, 0, sizeof(isapnp_t));

    io_sethandler(0x279, 1, 
		  NULL, NULL, NULL, isapnp_write_addr, NULL, NULL, dev);
    io_sethandler(0xa79, 1, 
    		  NULL, NULL, NULL, isapnp_write_data, NULL, NULL, dev);
